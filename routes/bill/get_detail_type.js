    ...
    handler(request, reply) {
        ...
        request.app.db.query(count_select, (err, countRes) => {
            if(err) {
                ...
                const select = `select d.* from bill b,bill_detail d,material m where b.id=d.bill_id and m.id = d.material_id  and ${where} and m.type='${request.query.type}' and d.material_id is not null and  b.id=${request.query.id} order by d.recommend desc`;
