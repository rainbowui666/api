...
module.exports = {
    ...
    handler(request, reply) {
        const select = `select c.*,(select IFNULL(nickname,name) from user where id=c.user_id) user_name from cart c where status=1 and is_confirm=1 and sum<>0 and group_bill_id=${request.query.group_bill_id} `;
