...
module.exports = {
    ...
    handler(request, reply) {
        const decodeImg = cache.get("user"+request.query.id);
        if(decodeImg){
            reply(decodeImg).type('image/png');
        }else{
            const select = `select headimgurl from user where id=${request.query.id};`;
            request.app.db.query(select, (err, res) => {
                if(err) {
                    request.log(['error'], err);
                    reply(Boom.serverUnavailable(config.errorMessage));
                } else {
                    if(_.isEmpty(res[0].headimgurl)){
                        ...
                        const req = http.request(options, function (resUrl) {  
                            resUrl.on('data', function (chunk) {  
                                
                                const decodeImg = new Buffer(chunk.toString("base64"), 'base64');
                                cache.put("user"+request.query.id, decodeImg);
                                reply(decodeImg).type('image/jpg');
                            });  
                        });  
