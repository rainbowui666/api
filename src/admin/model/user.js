...
module.exports = class extends think.Model {
  ...
  // 最近一个月销售记录
  // select (select name from material where id=b.material_id) name,count(b.material_id) count from  cart c,cart_detail cd,bill_detail b where cd.cart_id=c.id and cd.bill_detail_id=b.id and c.insert_date >DATE_SUB(CURDATE(), INTERVAL 1 MONTH) and b.material_id >0  group by b.material_id order by count desc;
};
