...
module.exports = class extends Base {
  ...
  async categoryAllAction() {
    ...
    _.each(categorys, (category) => {
      const type = _.filter(types, (type) => {
        return type.parent_id === category.id;
      });
      category['types'] = type;
  ...
  async getAction() {
    ...
    const category = await this.model('category').where({code: material.category}).find();
    const type = await this.model('category').where({code: material.type}).find();
    const focus = await this.model('focus').where({'material_id': this.post('materialId'), 'user_id': userId}).find();
    material['focus_id'] = focus.id;
    material['category_name'] = category.name;
