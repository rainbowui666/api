...
module.exports = class extends Base {
  ...
  async getFullRegionName(provinceId, cityId, districtId) {
    const isFullRegion = await this.controller('region').checkFullRegion(provinceId, cityId, districtId);
    if (!isFullRegion) {
      return '';
    }

    const regionList = await this.model('region').limit(3).order({'id': 'asc'}).where({id: {'in': [provinceId, cityId, districtId]}}).select();
  ...
  async getRegionName(regionId) {
    return this.model('region').where({id: regionId}).getField('name', true);
  }
  ...
  async getRegionList(parentId) {
    return this.model('region').where({parent_id: parentId}).select();
  }
  ...
  async getRegionInfo(regionId) {
    return this.model('region').where({id: regionId}).find();
  }
};
