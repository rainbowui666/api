...
module.exports = class extends Base {
  ...
  async deleteAction() {
    ...
    // 删除当天的同一个商品的足迹
    const goods = await this.model('mall_footprint').where({user_id: userId, id: footprintId}).find();
    await this.model('mall_footprint').where({user_id: userId, goods_id: goods.goods_id}).delete();
   ...
   */
  async listAction() {
    const list = await this.model('mall_footprint')
