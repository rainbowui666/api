...
module.exports = class extends Base {
  async indexAction() {
    ...
    const historyKeywordList = await this.model('mall_search_history').distinct('keyword').where({ user_id: think.userId }).limit(10).getField('keyword');
  ...
  async clearhistoryAction() {
    await this.model('mall_search_history').where({ user_id: think.userId }).delete();
    return this.success();
  }
};
